...
interface SimplifiedStudentMark {
    id: number;
    studentName: string;
...
interface EditingMark extends SimplifiedStudentMark {
    tempMarks: number;
    tempGrade: string;
    tempStatus: number;
}
...
const TeacherDashboard: React.FC = () => {
    ...
    const [students, setStudents] = useState<SimplifiedStudentMark[]>([]);
    ...
    const filteredStudents = useMemo(() => {
        ...
        return students.filter(student =>
            ...
            student.studentAdmissionNo.toLowerCase().includes(query)
        );
    ...
    const fetchAdmissionDataHandler = useCallback(async (grade: string, classValue: string, year: string) => {
        if (!grade || !classValue || !year || !selectedSubject || !selectedExam) {
            setAdmissionData([]);
            setStudents([]);
            return;
        }

        const cacheKey = `${grade}-${classValue}-${year}-${selectedSubject}-${selectedExam}`;
        ...
        try {
            setLoading(true);
            const data = await fetchStudentMarksWithFilters(
                grade,
                classValue,
                year,
                selectedExam,
            ...
            const formattedStudents: SimplifiedStudentMark[] = data.map((item, index) => ({
                id: index + 1,
            ...
            showSnackbar(
                `Failed to load student marks: ${error instanceof Error ? error.message : 'An unknown error occurred'}`,
                'error'
            );
        } finally {
            setLoading(false);
        }
    }, [selectedSubject, selectedExam, showSnackbar]);

    const handleEditClick = useCallback((student: SimplifiedStudentMark) => {
        setEditingMark({
            ...student,
            tempMarks: student.marks,
            tempGrade: student.marksGrade,
            tempStatus: student.status
        });
    ...
    const handleEditSave = useCallback(async () => {
        ...
        try {
            // Validate marks
            if (editingMark.tempMarks < 0 || editingMark.tempMarks > 100) {
                showSnackbar('Marks must be between 0 and 100', 'warning');
                return;
            }

            // Update the student in the list
            const updatedStudent: SimplifiedStudentMark = {
                ...editingMark,
                marks: editingMark.tempMarks,
                marksGrade: editingMark.tempGrade,
                status: editingMark.tempStatus
            };

            setStudents(prev =>
                prev.map(s => s.id === editingMark.id ? updatedStudent : s)
            );
            ...
            showSnackbar(
                `Failed to update marks: ${error instanceof Error ? error.message : 'An unknown error occurred'}`,
                'error'
            );
        }
    }, [editingMark, showSnackbar]);
        ...
        <Box sx={{ display: "flex", width: "100%", minHeight: "100vh" }}>
            ...
            <Box sx={{ flexGrow: 1, overflowX: 'hidden', width: '100%' }}>
                <AppBar position="static" sx={{
                    ...
                    <Navbar title="Edit Marks" sidebarOpen={sidebarOpen} setSidebarOpen={setSidebarOpen} />
                </AppBar>
