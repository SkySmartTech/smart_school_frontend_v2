...
const TeacherDashboard: React.FC = () => {
  ...
  const handleAttendanceChange = useCallback(
    (studentId: GridRowId, attendance: "present" | "absent") => {
        ...
        prev.map((s) =>
          s.id === studentId
            ? {
                ...
                student_grade_value:
                  attendance === "absent" ? "" : s.student_grade_value,
              }
            : s
        )
      );
      ...
      setModifiedMarks((prevModified) => ({
        ...
        [studentId]: {
          ...
          student_grade_value:
            attendance === "absent"
              ? ""
              : prevModified[studentId]?.student_grade_value,
        },
      }));
    },
    []
  ...
  const validateExcelData = (data: ExcelRow[]): string | null => {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if (
        row["Attendance"] &&
        !["present", "absent"].includes(row["Attendance"].toLowerCase())
      ) {
        return `Row ${
          i + 1
        }: Invalid attendance value (should be present/absent)`;
      }
    }
  ...
  const handleExcelUpload = async (
    ...
    reader.onload = async (e) => {
      try {
        ...
        const transformedData: ExtendedStudentMark[] = jsonData.map(
          (row, index) => ({
            id: index + 1,
            ...
            month: row["Month"],
            year: row["Year"],
            attendance: (row["Attendance"]?.toLowerCase() || "present") as
              | "present"
              | "absent",
  ...
  const columns: GridColDef<ExtendedStudentMark>[] = useMemo(
    () => [
      ...
      },
      {
        ...
        editable: false,
        renderCell: (
          params: GridRenderCellParams<ExtendedStudentMark, string>
              ...
              {filteredStudents.length > 0 && (
                ...
                >
                  ...
                  >
                    {loading
                      ? "Submitting..."
                      : `Submit ${modifiedCount} ${isMobile ? "" : "Records"}`}
                  </Button>
